...
const IMG = {
  biryani: 'https://images.unsplash.com/photo-1722698030083-75d1d50cabe4?w=300&h=300&fit=crop&auto=format',
  chicken: 'https://images.unsplash.com/photo-1515931215890-366d3990cf8d?w=300&h=300&fit=crop&auto=format',
  shawarma: 'https://images.unsplash.com/photo-1529006557810-274b9b2fc783?w=300&h=300&fit=crop&auto=format',
  snack: 'https://images.unsplash.com/photo-1788621138170-6854f17c9e2e?w=300&h=300&fit=crop&auto=format',
  streetSnack: 'https://images.unsplash.com/photo-1760263051331-0e4d3dafc9ff?w=300&h=300&fit=crop&auto=format',
};

const userPosts = socialPosts.slice(0, 2);

const reviews = [
  { restaurant: 'Azad Hotel', food: 'Mutton Biriyani', rating: 5, comment: 'Absolutely legendary. The rice was perfectly cooked and the mutton was so tender.', date: '2 days ago' },
  { restaurant: 'Chai Chronicles', food: 'Masala Chai', rating: 5, comment: 'Found this hidden gem and it did not disappoint. The slow-brewed chai is unlike anything else in TVM.', date: '1 week ago' },
  { restaurant: 'Buhari Hotel', food: 'Chicken Fry', rating: 4, comment: 'Classic Buhari experience. Always consistent. The raw shallots with lime is the perfect finish.', date: '2 weeks ago' },
];
      ...
      <div className="bg-[#24221D] px-4 pt-12 pb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-white/60 text-xs font-body mt-1">🍗 Chicken fry evangelist · TVM local</p>
          </div>
        </div>
      ...
      <div className="mt-4">
        {activeTab === 'posts' && (
          <div className="grid grid-cols-2 gap-2 px-4">
            {[IMG.biryani, IMG.chicken, IMG.shawarma, IMG.snack, IMG.streetSnack, IMG.biryani].map((img, i) => (
              <div key={i} className="relative aspect-square rounded-[16px] overflow-hidden bg-[#F5E9C8]">
                ...
                <div className="absolute bottom-2 left-2 right-2">
                  <p className="text-white text-xs font-display font-700 line-clamp-1">
                    {['Mutton Biriyani', 'Chicken Fry', 'Shawarma', 'Pazham Pori', 'Masala Chai', 'Kerala Meals'][i]}
                  </p>
                  <p className="text-white/70 text-[10px] font-body">❤ {[342, 187, 89, 456, 891, 123][i]}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {activeTab === 'reviews' && (
          <div className="px-4 flex flex-col gap-3">
            {reviews.map((rev, i) => (
              <div key={i} className="bg-white rounded-[16px] p-4" style={{ boxShadow: '0 2px 8px rgba(36,34,29,0.06)' }}>
