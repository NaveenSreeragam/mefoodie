...
type Screen = 'home' | 'explore' | 'craving' | 'feed' | 'saved' | 'profile' | 'restaurant' | 'create-post';
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#E8DEC8' }}>
      ...
      >
        ...
        <div className="h-screen overflow-y-auto">
          ...
          {screen === 'explore' && (
            <ExploreScreen onRestaurantClick={openRestaurant} />
          )}
          {screen === 'craving' && (
            <CravingScreen onExploreClick={() => navTo('explore')} />
          )}
          {screen === 'feed' && (
            <FeedScreen
              onRestaurantClick={openRestaurant}
        ...
        {screen !== 'restaurant' && screen !== 'create-post' && (
          <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] px-4 pb-6 pt-2 pointer-events-none">
            <nav
              ...
              style={{ boxShadow: '0 -4px 24px rgba(36,34,29,0.12), 0 8px 32px rgba(36,34,29,0.1)' }}
            >
              {/* Home */}
              <NavButton
  ...
  return (
    ...
    >
      <span className={`transition-all ${active ? 'scale-110' : ''}`}>
        {active
          ? <span style={{ color: '#FFC928' }}>{icon}</span>
          : icon}
      </span>
      <span className={`text-[10px] font-display font-700 ${active ? 'text-[#FFC928]' : 'text-[#C4BDB3]'}`}>
        {label}
      </span>
    </button>
  );
