...
const allFilters = ['Open now', 'Under ₹150', 'Under ₹300', 'Nearby', '4.5+ Rating', 'Veg Only', '🌶 Spicy', 'Delivery', 'Dine-in', 'Late Night'];

const foodResults = [
  { name: 'Mutton Biriyani', restaurant: 'Azad Hotel', restaurantId: 'azad', price: 220, rating: 4.8, distance: '1.2 km', image: IMG.biryani, isVeg: false, isSpicy: true },
  { name: 'Chicken Fry', restaurant: 'Buhari Hotel', restaurantId: 'buhari', price: 140, rating: 4.9, distance: '0.8 km', image: IMG.chicken, isVeg: false, isSpicy: true },
  { name: 'Chicken Shawarma', restaurant: 'Shawarma King', restaurantId: 'shawarma-king', price: 80, rating: 4.6, distance: '1.5 km', image: IMG.shawarma, isVeg: false, isSpicy: false },
  { name: 'Chicken Porotta', restaurant: 'Buhari Hotel', restaurantId: 'buhari', price: 160, rating: 4.7, distance: '0.8 km', image: IMG.chicken, isVeg: false, isSpicy: false },
  { name: 'Pazham Pori', restaurant: 'SL Bakery', restaurantId: 'sl-bakery', price: 12, rating: 4.8, distance: '0.4 km', image: IMG.snack, isVeg: true, isSpicy: false },
  { name: 'Masala Chai + Vada', restaurant: 'Chai Chronicles', restaurantId: 'chai-chronicles', price: 35, rating: 4.9, distance: '2.1 km', image: IMG.streetSnack, isVeg: true, isSpicy: false },
];
...
export default function ExploreScreen({ onRestaurantClick }: Props) {
  ...
  const filteredFood = foodResults.filter(f =>
    !query || f.name.toLowerCase().includes(query.toLowerCase()) || f.restaurant.toLowerCase().includes(query.toLowerCase())
  );

  const filteredRestaurants = restaurants.filter(r =>
    !query || r.name.toLowerCase().includes(query.toLowerCase()) || r.cuisine.toLowerCase().includes(query.toLowerCase())
  );
  ...
  return (
    <div className="pb-28 screen-enter">
      ...
      <div className="flex gap-2 px-4 overflow-x-auto pb-1 mb-4">
        ...
        {allFilters.map(f => (
          <FilterPill key={f} label={f} active={activeFilters.includes(f)} onClick={() => toggleFilter(f)} />
        ))}
      </div>
      ...
      <div className="flex gap-1 mx-4 mb-4 bg-[#F5E9C8] p-1 rounded-full">
        {(['food', 'restaurants'] as const).map(tab => (
          ...
          >
            {tab === 'food' ? `🍽 Food (${filteredFood.length})` : `🏪 Restaurants (${filteredRestaurants.length})`}
          </button>
        ))}
      </div>
            ...
            filteredRestaurants.map(r => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[#8B8578] text-xs font-body mt-0.5 line-clamp-1">{r.cuisine}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <span className={`text-[10px] font-bold ${r.isOpen ? 'text-green-600' : 'text-red-400'}`}>
                      {r.isOpen ? '● Open' : '● Closed'}
                    </span>
                    <span className="text-[#8B8578] text-[10px]">{r.distance} · {r.priceRange}</span>
                    {r.isHiddenGem && <span className="bg-[#FFC928] text-[#24221D] text-[9px] font-display font-800 px-1.5 py-0.5 rounded-full">✦ GEM</span>}
                  </div>
                </div>
      ...
      {!query && (
        <div className="mt-8 px-4">
          <h3 className="font-display font-800 text-[#24221D] text-base mb-3">Browse by category</h3>
          <div className="grid grid-cols-3 gap-3">
            {foodCategories.map(cat => (
              ...
              >
                ...
                <span className="text-xs font-body text-[#24221D] font-500 text-center leading-tight">{cat.label}</span>
              </button>
            ))}
          </div>
        </div>
...
function EmptyState({ query }: { query: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4">
      ...
      <p className="font-display font-800 text-[#24221D] text-lg text-center">No results for "{query}"</p>
      <p className="text-[#8B8578] text-sm font-body mt-2 text-center">Try searching for chicken, biriyani, shawarma…</p>
    </div>
  );
}
