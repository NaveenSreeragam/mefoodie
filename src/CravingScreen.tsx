...
const surpriseResults = [
  { emoji: '🍛', name: 'Mutton Biriyani', place: 'Azad Hotel', price: '₹220', reason: 'Because it\'s lunchtime and this is always the right answer' },
  { emoji: '🍗', name: 'Kerala Chicken Fry', place: 'Buhari Hotel', price: '₹140', reason: 'Crispy, spicy, perfect — need we say more?' },
  { emoji: '🌯', name: 'Spicy Shawarma', place: 'Shawarma King', price: '₹90', reason: 'Late night energy with maximum flavour' },
  { emoji: '☕', name: 'Masala Chai + Vada', place: 'Chai Chronicles', price: '₹35', reason: 'The hidden gem that always surprises first-timers' },
  { emoji: '🥐', name: 'Pazham Pori & Chai', place: 'SL Bakery', price: '₹27', reason: 'The most Kerala thing you can do right now' },
];
  ...
  return (
    <div className="pb-28 screen-enter">
      ...
      <div className="px-4 pt-12 pb-5">
        ...
        <p className="text-[#8B8578] text-sm font-body mt-1.5">Pick a mood and MeFoodie will find your perfect match</p>
      </div>
        ...
        {cravingTypes.map(craving => (
          <button
            ...
            style={{
              ...
              boxShadow: selected === craving.id ? `0 6px 24px ${craving.color}40` : '0 2px 8px rgba(36,34,29,0.06)',
            }}
          >
            <span className="text-3xl block mb-2">{craving.emoji}</span>
            <p className={`font-display font-800 text-sm leading-tight ${selected === craving.id ? 'text-white' : 'text-[#24221D]'}`}>
              {craving.label}
            </p>
            <p className={`text-xs font-body mt-1 leading-relaxed ${selected === craving.id ? 'text-white/80' : 'text-[#8B8578]'}`}>
              {craving.desc}
            </p>
          </button>
        ))}
      ...
      <div className="px-4 mb-6">
        <div className="bg-[#24221D] rounded-[24px] p-5" style={{ boxShadow: '0 8px 32px rgba(36,34,29,0.2)' }}>
          ...
          {surprise && (
            <div className="mt-4 bg-white/10 backdrop-blur-sm rounded-[16px] p-4 fade-in">
              ...
              <p className="text-white/70 text-xs font-body mt-3 italic leading-relaxed">"{surprise.reason}"</p>
              <button
                onClick={onExploreClick}
      ...
      <div className="px-4">
        <h3 className="font-display font-800 text-[#24221D] text-base mb-3">Popular craving combos 🔥</h3>
