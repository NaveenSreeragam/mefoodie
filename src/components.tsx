...
import { StarIcon, MapPinIcon, BookmarkIcon, HeartIcon, FlameIcon, MessageIcon, ShareIcon } from './icons';
import type { Restaurant, MenuItem, FoodPost } from './data';

// ─── Rating Badge ─────────────────────────────────────────────────────────────
export function RatingBadge({ rating, size = 'sm' }: { rating: number; size?: 'sm' | 'md' }) {
  return (
    <span className={`inline-flex items-center gap-0.5 bg-[#24221D] text-[#FFC928] font-display font-bold rounded-full ${size === 'sm' ? 'px-2 py-0.5 text-[11px]' : 'px-3 py-1 text-sm'}`}>
  ...
  return (
    <button
      ...
      className={`flex-shrink-0 px-4 py-1.5 rounded-full text-sm font-body font-medium transition-all duration-150 ${
        active
          ? 'bg-[#24221D] text-[#FFC928]'
          : 'bg-white text-[#24221D] border border-[#F5E9C8]'
      }`}
    >
      {label}
    </button>
...
export function FoodCard({
  name, restaurant, price, rating, distance, image, isVeg, isSpicy, onSave, saved = false, onClick,
}: {
  name: string; restaurant: string; price: number; rating: number;
  distance: string; image: string; isVeg?: boolean; isSpicy?: boolean;
  onSave?: () => void; saved?: boolean; onClick?: () => void;
}) {
  return (
    <div
      onClick={onClick}
    ...
    >
      <div className="relative h-36 bg-[#F5E9C8]">
        ...
        >
          <BookmarkIcon size={15} filled={saved} className={saved ? 'text-[#FFC928]' : 'text-[#8B8578]'} />
        </button>
      ...
      <div className="p-2.5">
        <p className="font-display font-700 text-[#24221D] text-sm leading-tight line-clamp-1">{name}</p>
        ...
        {isVeg !== undefined && (
          <div className="mt-1.5">
            <span className={`w-4 h-4 rounded-sm border-2 flex items-center justify-center ${isVeg ? 'border-green-600' : 'border-red-500'}`}>
...
export function RestaurantCard({
  restaurant, onClick,
}: { restaurant: Restaurant; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
    ...
    >
      <div className="relative h-36 bg-[#F5E9C8]">
        ...
        <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between">
          ...
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${restaurant.isOpen ? 'bg-green-500 text-white' : 'bg-red-400 text-white'}`}>
            {restaurant.isOpen ? `Open · ${restaurant.openUntil}` : 'Closed'}
          </span>
        </div>
      </div>
      <div className="p-3">
        <p className="font-display font-800 text-[#24221D] text-sm leading-tight">{restaurant.name}</p>
        <p className="text-[#8B8578] text-[11px] font-body mt-0.5 line-clamp-1">{restaurant.cuisine}</p>
        <div className="flex items-center gap-2 mt-2">
          <span className="text-[#8B8578] text-[11px] flex items-center gap-0.5">
            <MapPinIcon size={10} /> {restaurant.distance}
          </span>
          <span className="text-[#8B8578] text-[11px]">{restaurant.priceRange} · avg ₹{restaurant.avgPrice}</span>
        </div>
        <div className="flex flex-wrap gap-1 mt-2">
          {restaurant.tags.slice(0, 2).map(tag => (
            <span key={tag} className="bg-[#FFF9ED] text-[#8B8578] text-[10px] px-2 py-0.5 rounded-full">{tag}</span>
          ))}
        </div>
      </div>
    </div>
...
// ─── Hidden Gem Card ──────────────────────────────────────────────────────────
export function HiddenGemCard({ restaurant, onClick }: { restaurant: Restaurant; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4">
        ...
        {restaurant.gemDescription && (
          <p className="text-white/80 text-[11px] font-body mt-2 line-clamp-2 leading-relaxed">{restaurant.gemDescription}</p>
        )}
        <div className="flex items-center gap-2 mt-2">
          <RatingBadge rating={restaurant.rating} />
...
// ─── Social Post Card ─────────────────────────────────────────────────────────
export function SocialPostCard({ post, onRestaurantClick }: { post: FoodPost; onRestaurantClick: (id: string) => void }) {
  const [liked, setLiked] = useState(post.isLiked);
  const [saved, setSaved] = useState(post.isSaved);
  const [likes, setLikes] = useState(post.likes);

  return (
    <div className="bg-white rounded-[20px] overflow-hidden" style={{ boxShadow: '0 2px 12px rgba(36,34,29,0.06)' }}>
      ...
      <div className="relative h-64 bg-[#F5E9C8]">
        ...
        <div className="absolute bottom-3 left-3 flex flex-wrap gap-1">
          {post.tags.map(tag => (
            <span key={tag} className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${tag === 'Hidden Gem' ? 'bg-[#FFC928] text-[#24221D]' : 'bg-white/90 text-[#24221D]'}`}>
              {tag === 'Hidden Gem' ? '✦ ' : ''}{tag}
            </span>
          ))}
        </div>
      ...
      <div className="p-3.5">
        ...
        >
          <div>
            ...
            <p className="text-[#FFC928] text-xs font-body font-600 underline decoration-dotted">{post.restaurant}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 mt-3 pt-3 border-t border-[#F5E9C8]">
          ...
          >
            <HeartIcon size={18} filled={liked} className={liked ? 'text-red-500' : 'text-[#8B8578]'} />
...
export function SectionHeader({ title, subtitle, onSeeAll }: { title: string; subtitle?: string; onSeeAll?: () => void }) {
  return (
    <div className="flex items-start justify-between px-4 mb-3">
      ...
      {onSeeAll && (
        <button onClick={onSeeAll} className="text-[#FFC928] text-sm font-display font-700 mt-0.5">See all</button>
      )}
    </div>
  );
}
...
export function MenuItemCard({ item }: { item: MenuItem }) {
  ...
  return (
    <div className={`flex gap-3 p-3 rounded-[16px] ${!item.isAvailable ? 'opacity-50' : ''} ${item.isPopular ? 'bg-[#FFF9ED]' : 'bg-white'}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start gap-1">
          <span className={`w-3.5 h-3.5 rounded-sm border-2 flex-shrink-0 mt-0.5 flex items-center justify-center ${item.isVeg ? 'border-green-600' : 'border-red-500'}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${item.isVeg ? 'bg-green-600' : 'bg-red-500'}`} />
          </span>
          <div className="flex-1 min-w-0">
            <p className="font-display font-700 text-[#24221D] text-sm leading-tight line-clamp-1">{item.name}</p>
          </div>
          {item.isSpicy && <FlameIcon size={12} className="text-orange-500 flex-shrink-0 mt-0.5" />}
        </div>
        <p className="text-[#8B8578] text-[11px] font-body mt-0.5 line-clamp-2 leading-relaxed">{item.description}</p>
        <div className="flex items-center justify-between mt-2">
...
// ─── Search Bar ───────────────────────────────────────────────────────────────
export function SearchBar({ placeholder, onFocus, value, onChange }: {
  placeholder?: string; onFocus?: () => void;
