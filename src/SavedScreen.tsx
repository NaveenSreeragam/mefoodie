        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          {savedCollections.map(col => (
            ...
            >
              ...
              <p className={`font-display font-700 text-sm ${activeCollection === col.id ? 'text-[#FFC928]' : 'text-[#24221D]'}`}>{col.name}</p>
              <p className={`text-xs font-body mt-0.5 ${activeCollection === col.id ? 'text-white/60' : 'text-[#8B8578]'}`}>{col.count} items</p>
            </button>
          ))}
          ...
          <button className="p-4 rounded-[16px] text-left border-2 border-dashed border-[#F5E9C8] flex flex-col items-center justify-center gap-2">
            ...
            <p className="font-display font-700 text-[#8B8578] text-sm text-center">New Collection</p>
          </button>
        </div>
      </div>
      ...
      <div className="px-4">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-display font-800 text-[#24221D] text-base">
            {activeCollection ? savedCollections.find(c => c.id === activeCollection)?.name : 'Recently Saved'}
          </h2>
        </div>
            ...
            {restaurants.map(r => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[#8B8578] text-xs font-body">{r.distance} · {r.priceRange}</p>
                </div>
