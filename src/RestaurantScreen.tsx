...
import { ChevronLeftIcon, MapPinIcon, PhoneIcon, NavigationIcon, BookmarkIcon, StarIcon, ClockIcon, FlameIcon } from './icons';
    ...
    <div className="pb-28 screen-enter">
      ...
      <div className="relative h-56 bg-[#F5E9C8]">
        ...
        <button
          ...
          style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.15)' }}
        >
          <BookmarkIcon size={18} filled={saved} className={saved ? 'text-[#FFC928]' : 'text-[#24221D]'} />
        </button>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          <h1 className="font-display font-900 text-white text-2xl leading-tight">{restaurant.name}</h1>
      ...
      <div className="bg-white px-4 py-3.5 flex items-center gap-4 border-b border-[#F5E9C8]">
        ...
        <span className={`text-xs font-bold flex items-center gap-1 ${restaurant.isOpen ? 'text-green-600' : 'text-red-400'}`}>
          <span className={`w-1.5 h-1.5 rounded-full ${restaurant.isOpen ? 'bg-green-500' : 'bg-red-400'}`} />
          {restaurant.isOpen ? `Open until ${restaurant.openUntil}` : 'Closed'}
        </span>
        <span className="text-[#8B8578] text-xs">{restaurant.priceRange} · ₹{restaurant.avgPrice} avg</span>
      </div>
      ...
      <div className="bg-white px-4 py-3.5 border-b border-[#F5E9C8]">
        ...
        <div className="flex gap-2">
          {[
            ...
            { icon: <BookmarkIcon size={15} className={saved ? 'text-[#FFC928]' : 'text-[#24221D]'} />, label: saved ? 'Saved' : 'Save' },
          ].map(({ icon, label }) => (
            <button
              key={label}
      ...
      <div className="flex gap-2 px-4 py-3 overflow-x-auto bg-white border-b border-[#F5E9C8]">
        {restaurant.tags.map(tag => (
          <span key={tag} className={`flex-shrink-0 text-xs font-body px-3 py-1 rounded-full ${tag === 'Hidden Gem' ? 'bg-[#FFC928] text-[#24221D] font-700' : 'bg-[#FFF9ED] text-[#8B8578]'}`}>
            {tag}
          </span>
        ))}
      </div>
        ...
        <div className="flex gap-2 px-4 overflow-x-auto pb-2 mb-3">
          {restaurant.menu.map(cat => (
            <button
              ...
              style={activeCategory !== cat.id ? { boxShadow: '0 1px 4px rgba(36,34,29,0.06)' } : {}}
            >
              {cat.name}
            </button>
          ))}
